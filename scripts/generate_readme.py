...
def entry_md(p, t, lang):
    """README 条目：封面 + 折叠的 prompt（代码块自带一键复制，无需跳转）。"""
    ...
    if imgs:
        lines += ["<div align=\"center\">", "",
                  f'<a href="{p.get("source","#")}"><img src="images/{imgs[0]}" width="380" loading="lazy" alt="{title}"></a>', "",
                  f'<a href="{p.get("source","#")}">{t["watch"]}</a>', "", "</div>", ""]
    # prompt 默认折叠，展开即是代码块（右上角一键复制），不用跳转
    open_p = "📋 点击展开 / 复制提示词" if lang == "zh" else "📋 Click to expand / copy prompt"
    lines += ["<details>", f"<summary><b>{open_p}</b></summary>", "",
              "```", p["prompt"].strip(), "```", "", "</details>", ""]
    if p.get("note"):
        lines += [f"> 💡 {p['note']}", ""]
